<template>
    <div class="container mt-4">
      <h2 class="mb-4">Asistentes con Solicitud de Suspensión</h2>
      ...
      <div v-else-if="asistentes.length === 0" class="alert alert-info">
        No hay asistentes con solicitud de suspensión.
      </div>
  ...
  const cargarAsistentesPendientes = async () => {
    try {
      loading.value = true;
      asistentes.value = await asistenteService.getAllAsistentesSuspendidos();
    } catch (error) {
  ...
  const aceptarAsistente = (asistente) => {
    mostrarConfirmacion(
      ...
      `¿Estás seguro de reactivar la cuenta de ${asistente.nombre}?`,
      async () => {
        try {
          loading.value = true;
          await asistenteService.updateSolicitudSuspension(asistente.id,false);
          alert(`Cuenta de ${asistente.nombre} reactivada correctamente`);
        } catch (error) {
          console.error('Error al reactivar asistente:', error);
  ...
  const eliminarAsistente = (asistente) => {
    mostrarConfirmacion(
      ...
      `¿Estás seguro de eliminar la cuenta de ${asistente.nombre}? Se eliminará también la declaración de responsabilidad`,
      async () => {
        try {
          loading.value = true;
