...
<template>
  <div class="waiting-room">
    ...
    <div class="participants-list">
      ...
      <ul>
        <li class="participant you">
          <span class="participant-name">Tú ({{ userName }}) {{ isAsistente ? '- Asistente' : '' }}</span>
          <span class="participant-status">Presente</span>
        </li>
        
        <!-- Mostrar primero los asistentes -->
        <template v-if="assistants.length > 0">
          <li class="participant-section">Asistentes</li>
          <li 
            class="participant" 
            v-for="participant in assistants" 
        ...
        <template v-if="regularUsers.length > 0">
          <li class="participant-section">Usuarios</li>
          <li 
            class="participant" 
            v-for="participant in regularUsers" 
            :key="participant.userId"
          >
            <span class="participant-name">{{ participant.userName }}</span>
            <span class="participant-status">Presente</span>
    ...
    <div class="actions">
      <!-- Botón para asistentes -->
      <button 
        v-if="isAsistente" 
        class="start-button" 
        @click="$emit('start-call')" 
        :disabled="regularUsers.length === 0"
      >
        Iniciar Asistencia Técnica
      </button>
      
      <!-- Botón para usuarios regulares -->
      <button 
        v-else 
        class="start-button" 
        disabled
    ...
    <div class="info-panel" v-if="showInfoPanel">
      <p v-if="isAsistente && regularUsers.length === 0">
        <i class="fas fa-info-circle"></i> 
        Esperando que se conecten usuarios para iniciar la asistencia técnica.
      </p>
      <p v-else-if="!isAsistente && assistants.length === 0">
        <i class="fas fa-info-circle"></i> 
        Esperando que un asistente técnico se conecte para ayudarte.
      </p>
      <p v-else-if="isAsistente && regularUsers.length > 0">
        <i class="fas fa-info-circle"></i> 
        Hay usuarios esperando asistencia. Puedes iniciar la videollamada.
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    assistants() {
      return this.participants.filter(p => p.userRole === 'asistente');
    },
    regularUsers() {
      return this.participants.filter(p => p.userRole !== 'asistente');
    }
